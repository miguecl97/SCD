   ...
   if(cond_clientes.get_nwt() < n_sillas)
 	{
      ...
      if(cond_clientes.get_nwt() != 0 || cond_silla.get_nwt() != 0)
		...
		m1.lock();
      cout << "Cliente numero " << cli << " se sienta en la silla de pelar." << endl;
		m1.unlock();
...
{ 
   if(cond_silla.get_nwt() == 0){
      ...
      cout << "Barbero se duerme...\n" << endl;
		cond_barbero.wait();                 // solo se duerme al barbero si se dan las condiciones, la llamada al siguiente cliente se hace en la funcion finCliente, que es cuando se libera la silla
		m1.unlock(); 
   }
...
{
   // crear monitor
   cout << "Solucion al problema de  la barberia con monitores SU, con " << clientes << " clientes y una capacidad en la sala de espera de " << n_sillas <<" sillas" << endl;
