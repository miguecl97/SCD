   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      if ( num_celdas_ocupadas == tam_vector ){ // si buffer lleno
		...
		if(tag_aceptable != etiq_cons){
			 if(cont % 2 != 0){
				 MPI_Probe( MPI_ANY_SOURCE, etiq_modo1, MPI_COMM_WORLD, &estado);
				 MPI_Recv( &valor, 1, MPI_INT, MPI_ANY_SOURCE, etiq_modo1, MPI_COMM_WORLD, &estado );
			 }else{
				MPI_Probe( MPI_ANY_SOURCE, etiq_modo2, MPI_COMM_WORLD, &estado);
      ...
      {
         case etiq_modo1: // si ha sido el productor: insertar en buffer
			...
			case etiq_modo2: // si ha sido el productor: insertar en buffer
            buffer[primera_libre] = valor ;
            primera_libre = (primera_libre+1) % tam_vector ;
            num_celdas_ocupadas++ ;
            cout << "Buffer ha recibido valor " << valor << endl ;
