...
void funcion_productor(int id_proceso)
{
   for ( unsigned int i= 0 ; i < num_itemsprod ; i++ )
   {
		...
		if(id_proceso % 2 == 0)
     		 MPI_Ssend( &valor_prod, 1, MPI_INT, id_buffer, etiq_par, MPI_COMM_WORLD );
		else
     		 MPI_Ssend( &valor_prod, 1, MPI_INT, id_buffer, etiq_impar, MPI_COMM_WORLD );
   }
}
// ---------------------------------------------------------------------
...
void funcion_buffer()
{
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      if ( num_celdas_ocupadas == tam_vector ) // si buffer lleno
		...
		MPI_Irecv( &valor, 1, MPI_INT, MPI_ANY_SOURCE, tag_aceptable, MPI_COMM_WORLD, &ticket_envio);
		if(contador % 2 != 0){
			if(estado.MPI_TAG == etiq_par)
				 MPI_Recv( &valor, 1, MPI_INT, MPI_ANY_SOURCE, etiq_impar, MPI_COMM_WORLD, &estado );
				 contador++;
		}else{
				if(estado.MPI_TAG == etiq_impar){
				MPI_Recv( &valor, 1, MPI_INT, MPI_ANY_SOURCE, etiq_par, MPI_COMM_WORLD, &estado );
				contador++;
				}
				}

		// MPI_Irecv( &valor, 1, MPI_INT, MPI_ANY_SOURCE, tag, MPI_COMM_WORLD, &estado);
